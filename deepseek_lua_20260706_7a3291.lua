-- // ========== ELITE HUB v1.0.0 | COMPLETE EDITION (FIXED) ==========
-- // discord.gg/5RuMCxK3u6
-- // FUCK YOU IF YOU STEAL THIS SHIT - REAL MEMBER COUNT 🥀
...
-- // FETCH REAL DISCORD MEMBER COUNT (NON-BLOCKING)
local memberCount = "9,338+"
local guildId = "1388575687203356672"

task.spawn(function()
    pcall(function()
        local response = game:HttpGet("https://discord.com/api/v9/guilds/" .. guildId .. "/widget.json")
        local data = HttpService:JSONDecode(response)
        if data and data.members then
            memberCount = tostring(#data.members)
        end
    end)
end)

-- // ========== PREMIUM LOADING SCREEN (NON-BLOCKING) ==========
...
loadingCard.Size = UDim2.new(0, 420, 0, 320)
loadingCard.Position = UDim2.new(0.5, -210, 0.5, -160)
loadingCard.BackgroundColor3 = Color3.fromRGB(15, 15, 25)
loadingCard.BackgroundTransparency = 0.1
...
logoIcon.Position = UDim2.new(0.5, -30, 0, 25)
...
logoText.Position = UDim2.new(0, 0, 0, 85)
...
subtitleText.Position = UDim2.new(0, 0, 0, 130)
...
barBg.Position = UDim2.new(0.075, 0, 0, 205)
...
progressText.Position = UDim2.new(0, 0, 0, 218)
...
versionText.Position = UDim2.new(0, 0, 0, 260)
versionText.BackgroundTransparency = 1
versionText.Text = "v1.0.0 | LOADING..."
...
-- // Update member count when loaded
task.spawn(function()
    while memberCount == "9,338+" do
        task.wait(0.5)
    end
...
skipBtn.Position = UDim2.new(0.5, -60, 0, 280)
...
local fadeTween = TweenService:Create(loadingGui, TweenInfo.new(0.5), {BackgroundTransparency = 1})
fadeTween:Play()
...
if isMobile then
    ...
    local function createTouchButton(text, position, callback)
        ...
        button.MouseButton1Down:Connect(function() callback(true) end)
        button.MouseButton1Up:Connect(function() callback(false) end)
        button.TouchStarted:Connect(function() callback(true) end)
        button.TouchEnded:Connect(function() callback(false) end)
    end
