-- ============ ELITE HUB - COMPLETE BLOX FRUITS ============
-- Made by marcus 🥀
...
local Window = Rayfield:CreateWindow({
    Name = "ELITE HUB 🥀",
    LoadingTitle = "Loading this dick...",
    LoadingSubtitle = "by marcus - Fuck you if you steal",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "EliteHub", 
        FileName = "BloxFruits"
...
local function StartCustomFarm()
    while CustomFarmEnabled do
        pcall(function()
            local Enemies = workspace:FindFirstChild("Enemies")
            if not Enemies then task.wait(0.5) return end
            
            local Character = LP.Character
            if not Character then task.wait(0.5) return end
            local RootPart = Character:FindFirstChild("HumanoidRootPart")
            if not RootPart then task.wait(0.5) return end
            ...
            for _, Enemy in pairs(Enemies:GetChildren()) do
                ...
                if string.lower(cleanEnemyName) == string.lower(CustomTargetName) and 
...
FarmTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then task.spawn(StartCustomFarm) end
    end,
})
...
FarmTab:CreateButton({
    ...
    Callback = function()
        pcall(function()
            local Enemies = workspace:FindFirstChild("Enemies")
            if not Enemies then print("❌ No enemies!") return end
